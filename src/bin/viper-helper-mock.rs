...
fn handle(request: Request, states: &mut BTreeMap<String, String>) -> Response {
...
fn serve_connection(
    ...
    let response = match serde_json::from_str::<Request>(&line) {
        Ok(request) => handle(request, states),
...
fn main() -> std::io::Result<()> {
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                if let Err(error) = serve_connection(stream, &mut states) {
